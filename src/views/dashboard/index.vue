...
export default {
  ...
  methods: {
    ...
    modifyHead() {
      const top = this.path.segments[3]
      const bot = this.path.segments[4]
      const v = top.point.subtract(bot.point).normalize()
    ...
    getCenter(p1, p2) {
      const v = p1.subtract(p2).divide(2)
      const c = p1.subtract(v)
      // c.selected = true
      const cc = new paper.Path.Circle({
        center: c,
        radius: 50
      })

      const neww = this.path.unite(cc)
      this.path.remove()
      this.path = neww
    },
    modifyTrail() {
      ...
      const top = this.path.segments[0]
      const bot = this.path.segments[7]
      // top.previous = bot
      // bot.next = top
      console.log('top---', top)
      console.log('bot---', bot)
    ...
    test() {
      ...
      const x = Math.sqrt(100 * 100 / 2)
      const top = getCirclePoint(-80, 100)
    ...
    initWorld() {
      ...
      // }
      const cir = new paper.Path.Circle({
        center: new paper.Point(0),
        radius: 100,
        strokeColor: 'black'
      })
