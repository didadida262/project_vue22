...
var paper = function(self, undefined) {
  ...
  var Base = new function() {
    ...
    var forEach = array.forEach || function(iter, bind) {
      for (var i = 0, l = this.length; i < l; i++) {
        iter.call(bind, this[i], i, this)
      }
    }

    var forIn = function(iter, bind) {
      for (var i in this) {
        if (this.hasOwnProperty(i)) { iter.call(bind, this[i], i, this) }
      }
    }

    var set = Object.assign || function(dst) {
    ...
    var each = function(obj, iter, bind) {
      if (obj) {
    ...
    function inject(dest, src, enumerable, beans, preserve) {
      ...
      function field(name, val) {
        ...
        var res = val
        var prev = preserve || isFunc && !val.base
        ...
        if (!preserve || !prev) {
          ...
          if (isFunc && beans !== false &&
						(bean = name.match(/^([gs]et|is)(([A-Z])(.*))$/))) { beansNames[bean[3].toLowerCase() + bean[4]] = bean[2] }
      ...
      if (src) {
        ...
        for (var name in beansNames) {
          ...
          var set = dest['set' + part]
          var get = dest['get' + part] || set && dest['is' + part]
  ...
  Base.inject({
    ...
    statics: {
          ...
          if (Array.isArray(obj1) && Array.isArray(obj2)) {
            ...
            while (length--) {
              ...
              if (!(obj2.hasOwnProperty(key) &&
							Base.equals(obj1[key], obj2[key]))) { return false }
            }
          }
          return true
        }
      ...
      read: function(list, start, options, amount) {
        ...
        var readIndex = proto._readIndex
        var begin = start || readIndex && list.__index || 0
      ...
      readList: function(list, start, options, amount) {
        ...
        var entry
        var begin = start || 0
        var end = amount ? begin + amount : list.length
      ...
      deserialize: function(json, create, _data, _setDictionary, _isRoot) {
        var res = json
        var isFirst = !_data
        var hasDictionary = isFirst && json && json.length &&
				json[0][0] === 'dictionary'
        _data = _data || {}
        if (Array.isArray(json)) {
          var type = json[0]
          var isDictionary = type === 'dictionary'
  ...
  var Emitter = {
    ...
    emit: function(type, event) {
      ...
      var setTarget = event && event.target && !event.currentTarget
    ...
    _installEvents: function(install) {
      ...
      var key = install ? 'install' : 'uninstall'
      if (types) {
        for (var type in handlers) {
          if (handlers[type].length > 0) {
            var entry = types[type]
            var func = entry && entry[key]
      ...
      inject: function inject(src) {
        ...
        if (events) {
          ...
          Base.each(events, function(entry, key) {
            ...
            var name = isString ? entry : key
            var part = Base.capitalize(name)
            var type = name.substring(2).toLowerCase()
  ...
  var PaperScope = Base.extend({
    ...
    initialize: function PaperScope() {
      ...
      if (!this.agent) {
        ...
        var platform = os === 'darwin' ? 'mac' : os
        var agent = proto.agent = proto.browser = { platform: platform }
  ...
  var CollisionDetection = {
    findItemBoundsCollisions: function(items1, items2, tolerance) {
      ...
      var bounds2 = !items2 || items2 === items1
        ? bounds1
        : getBounds(items2)
    ...
    findCurveBoundsCollisions: function(curves1, curves2, tolerance, bothAxis) {
      function getBounds(curves) {
        var min = Math.min
        var max = Math.max
        var bounds = new Array(curves.length)
      ...
      var bounds2 = !curves2 || curves2 === curves1
        ? bounds1
        : getBounds(curves2)
      if (bothAxis) {
        var hor = this.findBoundsCollisions(
          bounds1, bounds2, tolerance || 0, false, true)
        var ver = this.findBoundsCollisions(
    ...
    findBoundsCollisions: function(boundsA, boundsB, tolerance,
      ...
      var lengthA = boundsA.length
      var lengthAll = allBounds.length
      ...
      var allCollisions = new Array(lengthA)
      for (var i = 0; i < lengthAll; i++) {
        ...
        if (activeIndicesByPri1.length) {
          ...
          var index = binarySearch(activeIndicesByPri1, pri1, curPri1)
  ...
  var Numerical = new function() {
    ...
    function getDiscriminant(a, b, c) {
      ...
      if (abs(D) * 3 < E) {
        ...
        var q = a * c
        var dq = (ad[0] * cd[0] - q + ad[0] * cd[1] + ad[1] * cd[0]) +
						ad[1] * cd[1]
        D = (p - q) + (dp - dq)
      }
      return D
    ...
    return {
      ...
      solveQuadratic: function(a, b, c, roots, min, max) {
        ...
        var minB = min - EPSILON
        var maxB = max + EPSILON
        if (isFinite(x1) && (boundless || x1 > minB && x1 < maxB)) { roots[count++] = boundless ? x1 : clamp(x1, min, max) }
        if (x2 !== x1 &&
					isFinite(x2) && (boundless || x2 > minB && x2 < maxB)) { roots[count++] = boundless ? x2 : clamp(x2, min, max) }
        return count
      },

      solveCubic: function(a, b, c, d, roots, min, max) {
        var f = getNormalizationFactor(abs(a), abs(b), abs(c), abs(d))
        var x; var b1; var c2; var qd; var q
        if (f) {
          a *= f
          b *= f
          c *= f
        ...
        if (abs(a) < EPSILON) {
          ...
          var s = t < 0 ? -1 : 1
          var td = -qd / a
          var rd = td > 0 ? 1.324717957244746 * Math.max(r, sqrt(td)) : r
          var x0 = x - s * rd
          if (x0 !== x) {
            do {
              evaluate(x0)
        ...
        if (isFinite(x) && (count === 0 ||
					...
					(boundless || x > min - EPSILON && x < max + EPSILON)) { roots[count++] = boundless ? x : clamp(x, min, max) }
        return count
      }
    }
  }()
  ...
  var Rectangle = Base.extend({
    ...
    intersects: function() {
      ...
      var epsilon = Base.read(arguments) || 0
    ...
    expand: function() {
      ...
      var hor = amount.width
      var ver = amount.height
  ...
  var Matrix = Base.extend({
    ...
    scale: function() {
      var args = arguments
      var scale = Point.read(args)
      var center = Point.read(args, 0, { readNull: true })
    ...
    skew: function() {
      var args = arguments
      var skew = Point.read(args)
      var center = Point.read(args, 0, { readNull: true })
      var toRadians = Math.PI / 180
      var shear = new Point(Math.tan(skew.x * toRadians),
    ...
    _transformCorners: function(rect) {
      ...
      var coords = [x1, y1, x2, y1, x2, y2, x1, y2]
      return this._transformCoordinates(coords, coords, 4)
    },

    _transformBounds: function(bounds, dest, _dontNotify) {
      var coords = this._transformCorners(bounds)
      var min = coords.slice(0, 2)
      var max = min.slice()
      for (var i = 2; i < 8; i++) {
        var val = coords[i]
        var j = i & 1
  ...
  new function() {
    ...
    _initialize: function(props, point) {
      ...
      var internal = hasProps && props.internal === true
      var matrix = this._matrix = new Matrix()
      var project = hasProps && props.project || paper.project
    ...
    _changed: function(flags) {
      ...
      var cacheParent = this._parent || symbol
    ...
    setName: function(name) {
      ...
      if (name && owner) {
        ...
        var namedChildren = owner._namedChildren;
    ...
    isFullySelected: function() {
      var children = this._children
      var selected = !!(this._selection & 1)
  ...
  },
  {
    ...
    _getCachedBounds: function(matrix, options, noInternal) {
      ...
      var cacheKey = cacheItem && (!matrix || matrix.equals(_matrix)) &&
      ...
      var rect = res.rect || res
      var style = this._style
      var nonscaling = res.nonscaling || style.hasStroke() &&
    ...
    _getStrokeMatrix: function(matrix, options) {
      ...
      var mx = parent ? parent.getViewMatrix().invert() : matrix
    ...
    statics: {
      _updateBoundsCache: function(parent, item) {
        if (parent && item) {
          var id = item._id
          var ref = parent._boundsCache = parent._boundsCache || {
            ids: {},
            list: []
          }
          if (!ref.ids[id]) {
    ...
    getScaling: function() {
      var decomposed = this._decompose()
      var s = decomposed && decomposed.scaling
      return new LinkedPoint(s ? s.x : 1, s ? s.y : 1, this, 'setScaling')
    },

    setScaling: function() {
      var current = this.getScaling()
      var scaling = Point.read(arguments, 0, { clone: true, readNull: true })
    ...
    clone: function(options) {
      ...
      var children = this._children
      var insert = Base.pick(options ? options.insert : undefined,
        options === undefined || options === true)
      var deep = Base.pick(options ? options.deep : undefined, true)
    ...
    rasterize: function(arg0, arg1) {
      ...
      var scale = (resolution || this.getView().getResolution()) / 72
      var topLeft = bounds.getTopLeft().floor()
      var bottomRight = bounds.getBottomRight().ceil()
      var boundsSize = new Size(bottomRight.subtract(topLeft))
      var rasterSize = boundsSize.multiply(scale)
      raster.setSize(rasterSize, true)

      if (!rasterSize.isZero()) {
        var ctx = raster.getContext(true)
        var matrix = new Matrix().scale(scale).translate(topLeft.negate())
        ctx.save()
        matrix.applyToContext(ctx)
  ...
  new function() {
    ...
    function hitTestAll() {
      var args = arguments
      var point = Point.read(args)
      var options = HitResult.getOptions(args)
      var all = []
    ...
    _hitTest: function(point, options, parentViewMatrix) {
      ...
      var matrix = this._matrix
      var viewMatrix = parentViewMatrix
        ? parentViewMatrix.appended(matrix)
        : this.getGlobalMatrix().prepend(this.getView()._matrix)
      var tolerance = Math.max(options.tolerance, 1e-12)
      var tolerancePadding = options._tolerancePadding = new Size(
      ...
      var checkCenter = options.center
      var checkBounds = options.bounds
    ...
    statics: {
      _getItems: function _getItems(item, options, matrix, param, firstOnly) {
        if (!param) {
          ...
          var overlapping = obj && obj.overlapping
          var inside = obj && obj.inside
          var bounds = overlapping || inside
          var rect = bounds && Rectangle.read([bounds])
          param = {
            items: [],
        ...
        for (var i = 0, l = children && children.length; i < l; i++) {
          var child = children[i]
          var childMatrix = matrix && matrix.appended(child._matrix)
          var add = true
          if (rect) {
            ...
            if (!(rect.contains(bounds) ||
							param.overlapping && (bounds.contains(rect) ||
								param.path.intersects(child, childMatrix)))) { add = false }
          }
    ...
    insertChildren: function(index, items) {
      ...
      if (children && items && items.length > 0) {
        ...
        var project = this._project
        var notifySelf = project._changes
        for (var i = 0, l = items.length; i < l; i++) {
          var item = items[i]
          var name = item._name
          item._parent = this
    ...
    _insertAt: function(item, offset) {
      ...
      var res = item !== this && owner ? this : null
      if (res) {
    ...
    _removeNamed: function() {
      ...
      if (owner) {
        ...
        var index = namedArray ? namedArray.indexOf(this) : -1
        if (index !== -1) {
    ...
    isGroupedWith: function(item) {
      ...
      while (parent) {
        if (parent._parent &&
				...
				item.isDescendant(parent)) { return true }
        parent = parent._parent
      }
      return false
    }
    ...
    this[key] = function() {
      var args = arguments
      var value = (rotate ? Base : Point).read(args)
      var center = Point.read(args, 0, { readNull: true })
    ...
    transform: function(matrix, _applyRecursively, _setApplyMatrix) {
      var _matrix = this._matrix
      var transformMatrix = matrix && !matrix.isIdentity()
      var applyMatrix = (
      ...
      if (transformMatrix) {
        ...
        var style = this._style
        var fillColor = style.getFillColor(true)
        var strokeColor = style.getStrokeColor(true)
    ...
    fitBounds: function(rectangle, fill) {
      ...
      var newBounds = new Rectangle(new Point(),
    ...
    _setStyles: function(ctx, param, viewMatrix) {
      ...
      if (style.hasStroke()) {
        ...
        var strokeCap = style.getStrokeCap()
        ...
        if (paper.support.nativeDash) {
          ...
          var dashOffset = style.getDashOffset()
      ...
      if (style.hasShadow()) {
        ...
        var mx = viewMatrix._shiftless().prepend(
          new Matrix().scale(pixelRatio, pixelRatio))
        var blur = mx.transform(new Point(style.getShadowBlur(), 0))
        var offset = mx.transform(this.getShadowOffset())
    ...
    draw: function(ctx, param, parentStrokeMatrix) {
      ...
      var viewMatrix = param.viewMatrix
      var matrix = this._matrix
      var globalMatrix = matrices[matrices.length - 1].appended(matrix)
      ...
      var opacity = Numerical.clamp(this._opacity, 0, 1)
      var normalBlend = blendMode === 'normal'
      var nativeBlend = BlendMode.nativeModes[blendMode]
      var direct = normalBlend && opacity === 1 ||
					param.dontStart ||
					param.clip ||
					(nativeBlend || normalBlend && opacity < 1) &&
						this._canComposite()
      var pixelRatio = param.pixelRatio || 1
      var mainCtx; var itemOffset; var prevOffset
      if (!direct) {
      ...
      var strokeMatrix = parentStrokeMatrix
        ...
        : this._canScaleStroke && !this.getStrokeScaling(true) &&
					viewMatrix
      var clip = !direct && param.clipItem
      var transform = !strokeMatrix || clip
      if (direct) {
    ...
    _drawSelection: function(ctx, matrix, size, selectionItems, updateVersion) {
      var selection = this._selection
      var itemSelected = selection & 1
      var boundsSelected = selection & 2 ||
					itemSelected && this._selectBounds
      var positionSelected = selection & 4
      if (!this._drawSelected) { itemSelected = false }
      if ((itemSelected || boundsSelected || positionSelected) &&
				this._isUpdated(updateVersion)) {
        var layer
        var color = this.getSelectedColor(true) || (layer = this.getLayer()) &&
					layer.getSelectedColor(true)
        var mx = matrix.appended(this.getGlobalMatrix(true))
        var half = size / 2
        ...
        if (positionSelected) {
          ...
          var parent = this._parent
          var point = parent ? parent.localToGlobal(pos) : pos
          var x = point.x
          var y = point.y
          ctx.beginPath()
    ...
    removeOn: function(obj) {
      for (var name in obj) {
        if (obj[name]) {
          var key = 'mouse' + name
          var project = this._project
          var sets = project._removeSets = project._removeSets || {}
          sets[key] = sets[key] || {}
    ...
    tween: function(from, to, options) {
      ...
      var duration = options != null && (
        typeof options === 'number' ? options : options.duration
      )
      var tween = new Tween(this, from, to, duration, easing, start)
  ...
  var Group = Item.extend({
    ...
    _draw: function(ctx, param) {
      var clip = param.clip
      var clipItem = !clip && this._getClipItem()
  ...
  var Shape = Item.extend({
    ...
    _draw: function(ctx, param, viewMatrix, strokeMatrix) {
      ...
      var dontPaint = param.dontFinish || param.clip
      var untransformed = !strokeMatrix
      if (hasFill || hasStroke || dontPaint) {
        ...
        if (untransformed && isCircle) {
          ...
          var ry = isCircle ? radius : radius.height
          var size = this._size
          var width = size.width
          var height = size.height
          if (untransformed && type === 'rectangle' && rx === 0 && ry === 0) {
            ...
            var x = width / 2
            var y = height / 2
            var kappa = 1 - 0.5522847498307936
            var cx = rx * kappa
            var cy = ry * kappa
            var c = [
              -x, -y + ry,
    ...
    _getBounds: function(matrix, options) {
      ...
      var style = this._style
      var strokeWidth = options.stroke && style.hasStroke() &&
  ...
  new function() {
    function getCornerCenter(that, point, expand) {
      ...
      if (!radius.isZero()) {
        ...
        for (var q = 1; q <= 4; q++) {
          ...
          var corner = dir.multiply(halfSize)
          var center = corner.subtract(dir.multiply(radius))
    ...
    return {
      ...
      _hitTestSelf: function _hitTestSelf(point, options, viewMatrix,
        ...
        var hit = false
        var style = this._style
        var hitStroke = options.stroke && style.hasStroke()
        var hitFill = options.fill && style.hasFill()
        if (hitStroke || hitFill) {
          var type = this._type
          var radius = this._radius
          var strokeRadius = hitStroke ? style.getStrokeWidth() / 2 : 0
          var strokePadding = options._tolerancePadding.add(
            Path._getStrokePadding(strokeRadius,
              !style.getStrokeScaling() && strokeMatrix))
          if (type === 'rectangle') {
            var padding = strokePadding.multiply(2)
            var center = getCornerCenter(this, point, padding)
            if (center) {
              hit = isOnEllipseStroke(point.subtract(center.point),
                radius, strokePadding, center.quadrant)
            } else {
              var rect = new Rectangle(this._size).setCenter(0, 0)
              var outer = rect.expand(padding)
              var inner = rect.expand(padding.negate())
    ...
    statics: new function() {
      ...
      return {
        Circle: function() {
          var args = arguments
          var center = Point.readNamed(args, 'center')
          var radius = Base.readNamed(args, 'radius')
        ...
        Rectangle: function() {
          ...
          var radius = Size.min(Size.readNamed(args, 'radius'),
        ...
        Ellipse: function() {
          var args = arguments
          var ellipse = Shape._readEllipse(args)
  ...
  var Raster = Item.extend({
    ...
    setImage: function(image) {
      ...
      function emit(event) {
        ...
        var type = event && event.type || 'load'
    ...
    getAverageColor: function(object) {
      ...
      var sampleSize = 32
      var width = Math.min(bounds.width, sampleSize)
      var height = Math.min(bounds.height, sampleSize)
  ...
  var Segment = Base.extend({
    ...
    initialize: function Segment(arg0, arg1, arg2, arg3, arg4, arg5) {
      ...
      var point; var handleIn; var handleOut; var selection
      if (count > 0) {
    ...
    _serialize: function(options, dictionary) {
      ...
      var selection = this._selection
      var obj = selection || this.hasHandles()
      ...
      if (curves) {
        if ((!point || point === this._point || point === this._handleIn) &&
					(curve = index > 0 ? curves[index - 1] : path._closed
					  ? curves[curves.length - 1] : null)) { curve._changed() }
        if ((!point || point === this._point || point === this._handleOut) &&
					(curve = curves[index])) { curve._changed() }
      }
      path._changed(41)
    },
    ...
    getCurve: function() {
      ...
      if (path) {
        if (index > 0 && !path._closed &&
					index === path._segments.length - 1) { index-- }
    ...
    smooth: function(options, _first, _last) {
      ...
      var p0 = (prev || this)._point
      var p1 = this._point
      var p2 = (next || this)._point
      var d1 = p0.getDistance(p1)
      var d2 = p1.getDistance(p2)
      if (!type || type === 'catmull-rom') {
        var a = factor === undefined ? 0.5 : factor
        var d1_a = Math.pow(d1, a)
        var d1_2a = d1_a * d1_a
        var d2_a = Math.pow(d2, a)
        var d2_2a = d2_a * d2_a
        if (!_first && prev) {
          var A = 2 * d2_2a + 3 * d2_a * d1_a + d1_2a
          var N = 3 * d2_a * (d2_a + d1_a)
        ...
        if (prev && next) {
          ...
          var t = factor === undefined ? 0.4 : factor
          var k = t * d1 / (d1 + d2)
    ...
    interpolate: function(from, to, factor) {
      var u = 1 - factor
      var v = factor
      var point1 = from._point
      var point2 = to._point
      var handleIn1 = from._handleIn
      var handleIn2 = to._handleIn
      var handleOut2 = to._handleOut
      var handleOut1 = from._handleOut
      this._point._set(
    ...
    _transformCoordinates: function(matrix, coords, change) {
      var point = this._point
      var handleIn = !change || !this._handleIn.isZero()
        ? this._handleIn : null
      var handleOut = !change || !this._handleOut.isZero()
  ...
  var Curve = Base.extend({
    ...
    remove: function() {
      ...
      if (this._path) {
        ...
        var handleOut = segment2._handleOut
    ...
    divideAtTime: function(time, _setHandles) {
      ...
      if (time >= tMin && time <= tMax) {
        ...
        var seg2 = this._segment2
        var path = this._path
        if (setHandles) {
          seg1._handleOut._set(left[2] - left[0], left[3] - left[1])
          seg2._handleIn._set(right[4] - right[6], right[5] - right[7])
        }
        var x = left[6]; var y = left[7]
    ...
    statics: {
      getValues: function(segment1, segment2, matrix, straight) {
        ...
        var h1 = segment1._handleOut
        var h2 = segment2._handleIn
        var p2 = segment2._point
      ...
      subdivide: function(v, t) {
        var x0 = v[0]; var y0 = v[1]
        var x1 = v[2]; var y1 = v[3]
        var x2 = v[4]; var y2 = v[5]
        var x3 = v[6]; var y3 = v[7]
        if (t === undefined) { t = 0.5 }
        var u = 1 - t
        var x4 = u * x0 + t * x1; var y4 = u * y0 + t * y1
        var x5 = u * x1 + t * x2; var y5 = u * y1 + t * y2
        var x6 = u * x2 + t * x3; var y6 = u * y2 + t * y3
        var x7 = u * x4 + t * x5; var y7 = u * y4 + t * y5
        var x8 = u * x5 + t * x6; var y8 = u * y5 + t * y6
        var x9 = u * x7 + t * x8; var y9 = u * y7 + t * y8
        return [
      ...
      getMonoCurves: function(v, dir) {
        ...
        if ((o0 >= o1) === (o1 >= o2) && (o1 >= o2) === (o2 >= o3) ||
          ...
          var tMax = 1 - tMin
          var roots = []
          var n = Numerical.solveQuadratic(a, b, c, roots, tMin, tMax)
          if (!n) {
            curves.push(v)
          } else {
            roots.sort()
            var t = roots[0]
            var parts = Curve.subdivide(v, t)
            curves.push(parts[0])
            if (n > 1) {
              t = (roots[1] - t) / (1 - t)
      ...
      _addBounds: function(v0, v1, v2, v3, coord, padding, min, max, roots) {
        ...
        if (v0 < minPad || v1 < minPad || v2 < minPad || v3 < minPad ||
          ...
          if (v1 < v0 != v1 < v3 && v2 < v0 != v2 < v3) {
            ...
            var b = 2 * (v0 + v2) - 4 * v1
            var c = v1 - v0
            var count = Numerical.solveQuadratic(a, b, c, roots)
            var tMin = 1e-8
            var tMax = 1 - tMin
            add(v3, 0)
            for (var i = 0; i < count; i++) {
    ...
    getNearestLocation: function() {
      ...
      var t = Curve.getNearestTime(values, point)
      var pt = Curve.getPoint(values, t)
  ...
  new function() {
    function getLengthIntegrand(v) {
      ...
      var ax = 9 * (x1 - x2) + 3 * (x3 - x0)
      var bx = 6 * (x0 + x2) - 12 * x1
      var cx = 3 * (x1 - x0)

      var ay = 9 * (y1 - y2) + 3 * (y3 - y0)
      var by = 6 * (y0 + y2) - 12 * y1
      var cy = 3 * (y1 - y0)
    ...
    return { statics: {
      ...
      classify: function(v) {
        ...
        var x2 = v[4]; var y2 = v[5]
        var x3 = v[6]; var y3 = v[7]
        var a1 = x0 * (y3 - y2) + y0 * (x2 - x3) + x3 * y2 - y3 * x2
        var a2 = x1 * (y0 - y3) + y1 * (x3 - x0) + x0 * y3 - y0 * x3
        var a3 = x2 * (y1 - y0) + y2 * (x0 - x1) + x1 * y0 - y1 * x0
        var d3 = 3 * a3
        var d2 = d3 - a2
        var d1 = d2 - a2 + a1
        var l = Math.sqrt(d1 * d1 + d2 * d2 + d3 * d3)
        var s = l !== 0 ? 1 / l : 0
        var isZero = Numerical.isZero
        var serpentine = 'serpentine'
        d1 *= s
        d2 *= s
        d3 *= s
      ...
      getTimeAt: function(v, offset, start) {
        ...
        var a = forward ? start : 0
        var b = forward ? 1 : start
        var ds = getLengthIntegrand(v)
        var rangeLength = Curve.getLength(v, a, b, ds)
        var diff = abs(offset) - rangeLength
        if (abs(diff) < epsilon) {
          return forward ? b : a
  ...
  new function() {
    function addLocation(locations, include, c1, t1, c2, t2, overlap) {
      ...
      if (t1 !== null && t1 >= (excludeStart ? tMin : 0) &&
        ...
        if (t2 !== null && t2 >= (excludeEnd ? tMin : 0) &&
          ...
          var loc2 = new CurveLocation(c2, t2, null, overlap)
    ...
    function addCurveIntersections(v1, v2, c1, c2, locations, include, flip,
      ...
      var q0x = v2[0]; var q0y = v2[1]; var q3x = v2[6]; var q3y = v2[7]
    ...
    function getConvexHull(dq0, dq1, dq2, dq3) {
      ...
      var p2 = [2 / 3, dq2]
      var p3 = [1, dq3]
      var dist1 = dq1 - (2 * dq0 + dq3) / 3
      var dist2 = dq2 - (dq0 + 2 * dq3) / 3
      var hull
      if (dist1 * dist2 < 0) {
    ...
    function addCurveLineIntersections(v1, v2, c1, c2, locations, include,
      flip) {
      var x1 = v2[0]; var y1 = v2[1]
      var x2 = v2[6]; var y2 = v2[7]
      var roots = getCurveLineIntersections(v1, x1, y1, x2 - x1, y2 - y1)
      for (var i = 0, l = roots.length; i < l; i++) {
        var t1 = roots[i]
        var p1 = Curve.getPoint(v1, t1)
        var t2 = Curve.getTimeOf(v2, p1)
        if (t2 !== null) {
          addLocation(locations, include,
        ...
        if (overlaps) {
          ...
          if (!straight || locations.length === before) {
            for (var i = 0; i < 4; i++) {
              ...
              var i1 = t1 * 6
              var i2 = t2 * 6
              var p1 = new Point(v1[i1], v1[i1 + 1])
              var p2 = new Point(v2[i2], v2[i2 + 1])
    ...
    function getIntersections(curves1, curves2, include, matrix1, matrix2,
      ...
      var values2 = self ? values1 : new Array(length2)
      var locations = []
    ...
    function getOverlaps(v1, v2) {
      ...
      var abs = Math.abs
      var getDistance = Line.getDistance
      ...
      var l1 = flip ? v2 : v1
      var l2 = flip ? v1 : v2
      var px = l1[0]; var py = l1[1]
      var vx = l1[6] - px; var vy = l1[7] - py
      ...
      for (var i = 0; i < 4 && pairs.length < 2; i++) {
        ...
        var i2 = i1 ^ 1
        var t1 = i >> 1
        var t2 = Curve.getTimeOf(v[i1], new Point(
          v[i2][t1 ? 6 : 0],
          v[i2][t1 ? 7 : 1]))
        if (t2 != null) {
      ...
      if (pairs.length !== 2) {
        ...
        var o2 = Curve.getPart(v2, pairs[0][1], pairs[1][1])
        if (abs(o2[2] - o1[2]) > geomEpsilon ||
				abs(o2[3] - o1[3]) > geomEpsilon ||
				abs(o2[4] - o1[4]) > geomEpsilon ||
				abs(o2[5] - o1[5]) > geomEpsilon) { pairs = null }
      }
      return pairs
    }
    ...
    function getTimesWithTangent(v, tangent) {
      ...
      var x2 = v[4]; var y2 = v[5]
      var x3 = v[6]; var y3 = v[7]
      var normalized = tangent.normalize()
      var tx = normalized.x
      var ty = normalized.y
    ...
    return {
      getIntersections: function(curve) {
        ...
        var v2 = curve && curve !== this && curve.getValues()
  ...
  var CurveLocation = Base.extend({
    ...
    divide: function() {
      var curve = this.getCurve()
      var res = curve && curve.divideAtTime(this.getTime())
      if (res) {
    ...
    split: function() {
      ...
      var res = curve && curve.splitAtTime(this.getTime())
      if (res) {
    ...
    equals: function(loc, _ignoreOther) {
      ...
      if (!res && loc instanceof CurveLocation) {
        ...
        if (p1 === p2) {
          var abs = Math.abs
          var epsilon = 1e-7
          var diff = abs(this.getOffset() - loc.getOffset())
          var i1 = !_ignoreOther && this._intersection
          var i2 = !_ignoreOther && loc._intersection
          res = (diff < epsilon ||
    ...
    isCrossing: function() {
      ...
      var c1 = c2 && t1 < tMin ? c2.getPrevious() : c2
      var c4 = inter.getCurve()
      var c3 = c4 && t2 < tMin ? c4.getPrevious() : c4
      ...
      function addOffsets(curve, end) {
        ...
        var roots = Curve.classify(v).roots || Curve.getPeaks(v)
        var count = roots.length
        var offset = Curve.getLength(v,
      ...
      var v2 = t1Inside ? c2.getTangentAtTime(t1)
        : c2.getPointAt(offset).subtract(pt)
      var v1 = t1Inside ? v2.negate()
        : c1.getPointAt(-offset).subtract(pt)
      var v4 = t2Inside ? c4.getTangentAtTime(t2)
        : c4.getPointAt(offset).subtract(pt)
      var v3 = t2Inside ? v4.negate()
  ...
  var PathItem = Item.extend({
    ...
    interpolate: function(from, to, factor) {
      ...
      var name = isPath ? '_segments' : '_children'
      var itemsFrom = from[name]
      var itemsTo = to[name]
      var items = this[name]
  ...
  var Path = PathItem.extend({
    ...
    getPathData: function(_matrix, _precision) {
      ...
      var f = new Formatter(_precision)
    ...
    _add: function(segs, index) {
      ...
      if (curves) {
        ...
        var start = index > 0 && index + amount - 1 === total ? index - 1
          : index
        var insert = start
    ...
    splitAt: function(location) {
      var loc = this.getLocationAt(location)
      var index = loc && loc.index
      var time = loc && loc.time
      var tMin = 1e-8
      var tMax = 1 - tMin
      if (time > tMax) {
        index++
    ...
    split: function(index, time) {
      var curve
      var location = time === undefined ? index
    ...
    reduce: function(options) {
      var curves = this.getCurves()
      var simplify = options && options.simplify
      var tolerance = simplify ? 1e-7 : 0
      for (var i = curves.length - 1; i >= 0; i--) {
        var curve = curves[i]
        if (!curve.hasHandles() && (!curve.hasLength(tolerance) ||
					simplify && curve.isCollinear(curve.getNext()))) { curve.remove() }
      }
      return this
    },
    ...
    smooth: function(options) {
      ...
      if (/^(?:asymmetric|continuous)$/.test(type)) {
        ...
        var min = Math.min
        var amount = to - from + 1
        var n = amount - 1
        var padding = loop ? min(amount, 4) : 1
        ...
        for (var i = 1; i < n; i++) {
          var internal = i < n_1
          var a = internal ? 1 : asymmetric ? 1 : 2
          var b = internal ? 4 : asymmetric ? 2 : 7
          var u = internal ? 4 : asymmetric ? 3 : 8
          var v = internal ? 2 : asymmetric ? 0 : 1
          var m = a / f
          f = rf[i] = b - m
    ...
    toShape: function(insert) {
      ...
      function isCollinear(i, j) {
        var seg1 = segments[i]
        var seg2 = seg1.getNext()
        var seg3 = segments[j]
        var seg4 = seg3.getNext()
      ...
      function isOrthogonal(i) {
        var seg2 = segments[i]
        var seg1 = seg2.getPrevious()
        var seg3 = seg2.getNext()
        return seg1._handleOut.isZero() && seg2._handleIn.isZero() &&
      ...
      function isArc(i) {
        ...
        if (handle1.isOrthogonal(handle2)) {
          var pt1 = seg1._point
          var pt2 = seg2._point
          var corner = new Line(pt1, handle1, true).intersect(
    ...
    _hitTestSelf: function(point, options, viewMatrix, strokeMatrix) {
      ...
      var closed = this._closed
      var tolerancePadding = options._tolerancePadding
      var strokePadding = tolerancePadding
      var join; var cap; var miterLimit
      var area; var loc; var res
      var hitStroke = options.stroke && style.hasStroke()
      var hitFill = options.fill && style.hasFill()
      var hitCurves = options.curves
      var strokeRadius = hitStroke
      ...
      if (options.ends && !options.segments && !closed) {
        if (res = checkSegmentPoints(segments[0], true) ||
					checkSegmentPoints(segments[numSegments - 1], true)) { return res }
  ...
  new function() {
    ...
    return {
      _draw: function(ctx, param, viewMatrix, strokeMatrix) {
        ...
        var hasStroke = style.hasStroke()
        var dashArray = style.getDashArray()
        var dashLength = !paper.support.nativeDash && hasStroke &&
          ...
          if (hasStroke) {
            if (dashLength) {
              ...
              var flattener = new PathFlattener(this, 0.25, 32, false,
                strokeMatrix)
              var length = flattener.length
              var from = -style.getDashOffset(); var to
              var i = 0
              while (from > 0) {
  ...
  new function() {
    ...
    return {
      ...
      cubicCurveTo: function() {
        var args = arguments
        var handle1 = Point.read(args)
        var handle2 = Point.read(args)
        var to = Point.read(args)
      ...
      curveTo: function() {
        var args = arguments
        var through = Point.read(args)
        var to = Point.read(args)
        var t = Base.pick(Base.read(args), 0.5)
        var t1 = 1 - t
        var current = getCurrentSegment(this)._point
        var handle = through.subtract(current.multiply(t1 * t1))
      ...
      arcTo: function() {
        ...
        var sqrt = Math.sqrt
        var current = getCurrentSegment(this)
        var from = current._point
        var to = Point.read(args)
        var through
        var peek = Base.peek(args)
        var clockwise = Base.pick(peek, true)
        var center; var extent; var vector; var matrix
        if (typeof clockwise === 'boolean') {
          var middle = from.add(to).divide(2)
          var through = middle.add(middle.subtract(from).rotate(
            clockwise ? -90 : 90))
          ...
          var clockwise = !!Base.read(args)
          var large = !!Base.read(args)
          var middle = from.add(to).divide(2)
          var pt = from.subtract(middle).rotate(-rotation)
          var x = pt.x
          var y = pt.y
        ...
        if (through) {
          ...
          var l2 = new Line(through.add(to).divide(2),
            to.subtract(through).rotate(90), true)
          var line = new Line(from, to)
          var throughSide = line.getSide(through)
        ...
        if (extent) {
          var epsilon = 1e-7
          var ext = abs(extent)
          var count = ext >= 360 ? 4 : Math.ceil((ext - epsilon) / 90)
          var inc = extent / count
          var half = inc * Math.PI / 360
          var z = 4 / 3 * Math.sin(half) / (1 + Math.cos(half))
          var segments = []
          for (var i = 0; i <= count; i++) {
            var pt = to
            var out = null
      ...
      arcBy: function() {
        var args = arguments
        var current = getCurrentSegment(this)._point
        var point = current.add(Point.read(args))
        var clockwise = Base.pick(Base.peek(args), true)
        if (typeof clockwise === 'boolean') {
    ...
    statics: {
      getBounds: function(segments, closed, path, matrix, options, strokePadding) {
        ...
        var coords = new Array(6)
        var prevCoords = first._transformCoordinates(matrix, new Array(6))
      ...
      getStrokeBounds: function(segments, closed, path, matrix, options) {
        ...
        var stroke = style.hasStroke()
        var strokeWidth = style.getStrokeWidth()
        var strokeMatrix = stroke && path._getStrokeMatrix(matrix, options)
        var strokePadding = stroke && Path._getStrokePadding(strokeWidth,
          strokeMatrix)
        ...
        var miterLimit = style.getMiterLimit()
        var joinBounds = new Rectangle(new Size(strokePadding))
      ...
      _getStrokePadding: function(radius, matrix) {
        ...
        var ver = new Point(0, radius).transform(matrix)
        var phi = hor.getAngleInRadians()
        var a = hor.getLength()
        var b = ver.getLength()
        var sin = Math.sin(phi)
        var cos = Math.cos(phi)
        var tan = Math.tan(phi)
        var tx = Math.atan2(b * tan, a)
        var ty = Math.atan2(b, tan * a)
      ...
      _addBevelJoin: function(segment, join, radius, miterLimit, matrix,
        ...
        var point = curve2.getPoint1().transform(matrix)
        var normal1 = curve1.getNormalAtTime(1).multiply(radius)
          .transform(strokeMatrix)
        var normal2 = curve2.getNormalAtTime(0).multiply(radius)
          .transform(strokeMatrix)
      ...
      _addSquareCap: function(segment, cap, radius, matrix, strokeMatrix,
        ...
        var loc = segment.getLocation()
        var normal = loc.getNormal()
      ...
      getHandleBounds: function(segments, closed, path, matrix, options) {
        var style = path.getStyle()
        var stroke = options.stroke && style.hasStroke()
        var strokePadding
        var joinPadding
        if (stroke) {
          var strokeMatrix = path._getStrokeMatrix(matrix, options)
          var strokeRadius = style.getStrokeWidth() / 2
          var joinRadius = strokeRadius
        ...
        for (var i = 0, l = segments.length; i < l; i++) {
          ...
          for (var j = 0; j < 6; j += 2) {
            ...
            var paddingX = padding ? padding[0] : 0
            var paddingY = padding ? padding[1] : 0
            var x = coords[j]
            var y = coords[j + 1]
            var xn = x - paddingX
            var xx = x + paddingX
  ...
  Path.inject({ statics: new function() {
    ...
    function createPath(segments, closed, args) {
      ...
      var path = new Path(props && props.insert == false && Item.NO_INSERT)
      path._add(segments)
      path._closed = closed
    ...
    return {
      ...
      Rectangle: function() {
        var args = arguments
        var rect = Rectangle.readNamed(args, 'rectangle')
        var radius = Size.readNamed(args, 'radius', 0,
          { readNull: true })
        var bl = rect.getBottomLeft(true)
        var tl = rect.getTopLeft(true)
        var tr = rect.getTopRight(true)
        var br = rect.getBottomRight(true)
        var segments
      ...
      Arc: function() {
        var args = arguments
        var from = Point.readNamed(args, 'from')
        var through = Point.readNamed(args, 'through')
        var to = Point.readNamed(args, 'to')
        var props = Base.getNamed(args)
        var path = new Path(props && props.insert == false &&
						Item.NO_INSERT)
        path.moveTo(from)
      ...
      Star: function() {
        var args = arguments
        var center = Point.readNamed(args, 'center')
        var points = Base.readNamed(args, 'points') * 2
        var radius1 = Base.readNamed(args, 'radius1')
        var radius2 = Base.readNamed(args, 'radius2')
        var step = 360 / points
        var vector = new Point(0, -1)
        var segments = new Array(points)
  ...
  new function() {
    ...
    function(key) {
      ...
      moveTo: function() {
        ...
        var path = current && current.isEmpty() ? current
      ...
      moveBy: function() {
        ...
        var last = current && current.getLastSegment()
        var point = Point.read(arguments)
  ...
  PathItem.inject(new function() {
    ...
    function traceBoolean(path1, path2, operation, options) {
      if (options && (options.trace == false || options.stroke) &&
				/^(subtract|intersect)$/.test(operation)) { return splitBoolean(path1, path2, operation) }
      var _path1 = preparePath(path1, true)
      var _path2 = path2 && path1 !== path2 && preparePath(path2, true)
      var operator = operators[operation]
      operator[operation] = true
      if (_path2 && (operator.subtract || operator.exclude) ^
				(_path2.isClockwise() ^ _path1.isClockwise())) { _path2.reverse() }
      var crossings = divideLocations(CurveLocation.expand(
        _path1.getIntersections(_path2, filterIntersection)))
      var paths1 = getPaths(_path1)
      var paths2 = _path2 && getPaths(_path2)
      var segments = []
      var curves = []
      var paths
      ...
      if (crossings.length) {
        ...
        for (var i = 0; i < curves.length; i++) {
          var curve = curves[i]
          var id = curve._path._id
          var map = curveCollisionsMap[id] = curveCollisionsMap[id] || {}
          map[curve.getIndex()] = {
    ...
    function splitBoolean(path1, path2, operation) {
      ...
      var crossings = _path1.getIntersections(_path2, filterIntersection)
    ...
    function reorientPaths(paths, isInside, clockwise) {
      ...
      if (length) {
        ...
        for (var i = 0; i < length; i++) {
          ...
          var containerWinding = 0
          var indices = collisions[i]
          if (indices) {
            var point = null
    ...
    function getWinding(point, curves, dir, closed, dontFlip) {
      ...
      var ia = dir ? 1 : 0
      var io = ia ^ 1
      var pv = [point.x, point.y]
      var pa = pv[ia]
      var po = pv[io]
      var windingEpsilon = 1e-9
      var qualityEpsilon = 1e-6
      var paL = pa - windingEpsilon
      var paR = pa + windingEpsilon
      var windingL = 0
      var windingR = 0
      var pathWindingL = 0
      var pathWindingR = 0
      ...
      function addWinding(v) {
        ...
        var windingPrev = vPrev[io] > vPrev[io + 6] ? 1 : -1
        var a3Prev = vPrev[ia + 6]
        if (po !== o0) {
          if (a < paL) {
      ...
      function handleCurve(v) {
        ...
        if (po <= max(o0, o1, o2, o3) && po >= min(o0, o1, o2, o3)) {
          ...
          var a2 = v[ia + 4]
          var a3 = v[ia + 6]
          var monoCurves = paL > max(a0, a1, a2, a3) ||
								 paR < min(a0, a1, a2, a3)
    ...
    function propagateWinding(segment, path1, path2, curveCollisionsMap,
      ...
      for (var i = 0; i < offsets.length && winding.quality < 0.5; i++) {
        ...
        for (var j = 0, l = chain.length; j < l; j++) {
          var entry = chain[j]
          var curveLength = entry.length
          if (length <= curveLength) {
            var curve = entry.curve
            var path = curve._path
            var parent = path._parent
            var operand = parent instanceof CompoundPath ? parent : path
            var t = Numerical.clamp(curve.getTimeAt(length), tMin, tMax)
            var pt = curve.getPointAtTime(t)
            var dir = abs(curve.getTangentAtTime(t).y) < Math.SQRT1_2
            var wind = null
            if (operator.subtract && path2) {
              var otherPath = operand === path1 ? path2 : path1
              var pathWinding = otherPath._getWinding(pt, dir, true)
    ...
    function tracePaths(segments, operator) {
      ...
      function getCrossingSegments(segment, collectStarts) {
        ...
        function collect(inter, end) {
          while (inter && inter !== end) {
            ...
            var path = other && other._path
            if (path) {
              var next = other.getNext() || path.getFirstSegment()
              var nextInter = next._intersection
      ...
      for (var i = 0, l = segments.length; i < l; i++) {
        ...
        if (valid && seg._path._overlapsOnly) {
          var path1 = seg._path
          var path2 = seg._intersection._segment._path
    ...
    return {
      ...
      resolveCrossings: function() {
        ...
        var intersections = this.getIntersections(null, function(inter) {
        ...
        if (hasOverlaps) {
          ...
          for (var i = overlaps.length - 1; i >= 0; i--) {
            var overlap = overlaps[i]
            var path = overlap._path
            var seg = overlap._segment
        ...
        if (hasCrossings) {
          divideLocations(intersections, hasOverlaps && function(inter) {
            ...
            var curve2 = other._curve
            var seg2 = other._segment
      ...
      getInteriorPoint: function() {
        var bounds = this.getBounds()
        var point = bounds.getCenter(true)
    ...
    _get: function(offset) {
      ...
      for (; i < length; i++) {
        ...
        if (part.offset >= offset) {
          ...
          var prevOffset = prev ? prev.offset : 0
          return {
            index: part.index,
  ...
  var PointText = TextItem.extend({
    ...
    _getBounds: function(matrix, options) {
      ...
      var leading = style.getLeading()
      var width = this.getView().getTextWidth(style.getFontStyle(), lines)
      var x = 0
  ...
  var Color = Base.extend(new function() {
    ...
    var converters = {
      ...
      'rgb-hsl': function(r, g, b) {
        ...
        var min = Math.min(r, g, b)
        var delta = max - min
        var achromatic = delta === 0
        var h = achromatic ? 0
          : (max == r ? (g - b) / delta + (g < b ? 6 : 0)
            : max == g ? (b - r) / delta + 2
              : (r - g) / delta + 4) * 60
        var l = (max + min) / 2
        var s = achromatic ? 0 : l < 0.5
          ? delta / (max + min)
          : delta / (2 - max - min)
        return [h, s, l]
      },
    ...
    return Base.each(types, function(properties, type) {
      ...
      Base.each(properties, function(name, index) {
        var part = Base.capitalize(name)
        var hasOverlap = /^(hue|saturation)$/.test(name)
        var parser = componentParsers[type][index] = type === 'gradient'
          ? name === 'gradient'
            ? function(value) {
        ...
        if (!components) {
          ...
          if (values) {
            ...
            if (arg.constructor === Color) {
              ...
              var parsers = componentParsers[type]
      ...
      toString: function() {
        ...
        var parts = []
        var isGradient = this._type === 'gradient'
        var f = Formatter.instance
        for (var i = 0, l = properties.length; i < l; i++) {
      ...
      toCSS: function(hex) {
        ...
        var alpha = hex || this._alpha == null ? 1 : this._alpha
      ...
      toCanvasStyle: function(ctx, matrix) {
        ...
        var gradient = components[0]
        var stops = gradient._stops
        var origin = components[1]
        var destination = components[2]
        var highlight = components[3]
        var inverse = matrix && matrix.inverted()
        var canvasGradient
        if (inverse) {
  ...
  new function() {
    ...
    return Base.each(operators, function(operator, name) {
      this[name] = function(color) {
        ...
        var components2 = color._convert(type)
  ...
  var Style = Base.extend(new function() {
    ...
    var groupDefaults = Base.set({}, itemDefaults, {
    ...
    var textDefaults = Base.set({}, groupDefaults, {
      fillColor: new Color()
    })
    var flags = {
      strokeWidth: 193,
    ...
    Base.each(groupDefaults, function(value, key) {
      ...
      fields[set] = function(value) {
        var owner = this._owner
        var children = owner && owner._children
        var applyToChildren = children && children.length > 0 &&
    ...
    set: function(style) {
      ...
      var values = isStyle ? style._values : style
      if (values) {
    ...
    equals: function(style) {
      function compare(style1, style2, secondary) {
        ...
        var values2 = style2._values
        var defaults2 = style2._defaults
        for (var key in values1) {
          var value1 = values1[key]
          var value2 = values2[key]
          if (!(secondary && key in values2) && !Base.equals(value1,
            value2 === undefined ? defaults2[key] : value2)) { return false }
        }
        return true
      }
  ...
  var DomElement = new function() {
    function handlePrefix(el, name, set, value) {
      ...
      for (var i = 0; i < 6; i++) {
        ...
        var key = prefix ? prefix + suffix : name
        if (key in el) {
          if (set) {
            el[key] = value
          } else {
    ...
    return {
      ...
      getBounds: function(el, viewport) {
        ...
        var y = rect.top - (html.clientTop || body.clientTop || 0)
        if (!viewport) {
  ...
  var View = Base.extend(Emitter, {
    ...
    initialize: function View(project, element) {
      ...
      if (window && element) {
        ...
        if (PaperScope.hasAttribute(element, 'stats') &&
          ...
          var offset = DomElement.getOffset(element)
    ...
    _handleFrame: function() {
      ...
      var delta = this._last ? now - this._last : 0
      this._last = now
  ...
  new function() {
    ...
    var mousedown; var mousemove; var mouseup
    ...
    var docEvents = {
      mouseout: function(event) {
        var view = View._focused
        var target = DomEvent.getRelatedTarget(event)
    ...
    return {
      ...
      _handleMouseEvent: function(type, event, point) {
        ...
        var hitItems = itemEvents.native[type]
        var nativeMove = type === 'mousemove'
        ...
        var hit = hitItems && inView && view._project.hitTest(point, {
          tolerance: 0,
          fill: true,
          stroke: true
        })
        var hitItem = hit && hit.item || null
        var handle = false
        var mouse = {}
      ...
      _countItemEvent: function(type, sign) {
        var itemEvents = this._itemEvents
        var native = itemEvents.native
        var virtual = itemEvents.virtual
  ...
  var CanvasView = View.extend({
    ...
    initialize: function CanvasView(project, canvas) {
      ...
      if (!/^off|false$/.test(PaperScope.getAttribute(canvas, 'hidpi'))) {
        ...
        var backingStoreRatio = DomElement.getPrefixed(ctx,
    ...
    DomEvent.add(document, {
      ...
      keypress: function(event) {
        if (downKey) {
          ...
          var character = code >= 32 ? String.fromCharCode(code)
            : key.length > 1 ? '' : key
          if (key !== downKey) {
  ...
  var Tool = PaperScopeItem.extend({
    ...
    _handleMouseEvent: function(type, event, point, mouse) {
      ...
      function update(minDistance, maxDistance) {
        var pt = point
        var toolPoint = move ? tool._point : (tool._downPoint || pt)
        if (move) {
  ...
  var Tween = Base.extend(Emitter, {
    ...
    update: function(progress) {
      if (this.running) {
        ...
        for (var i = 0, l = keys && keys.length; i < l; i++) {
          var key = keys[i]
          var from = getValue(this._from[key])
          var to = getValue(this._to[key])
    ...
    _getState: function(state) {
      ...
      for (var i = 0, l = keys.length; i < l; i++) {
        var key = keys[i]
        var path = this._parsedKeys[key]
        var current = this._getProperty(path)
        var value
        if (state) {
  ...
  var Http = {
    request: function(options) {
      ...
      xhr.onerror = function() {
        var status = xhr.status
        var message = 'Could not load "' + options.url + '" (Status: ' +
						status + ')'
        if (options.onError) {
  ...
  var BlendMode = new function() {
    ...
    var max = Math.max
    var abs = Math.abs
    var sr; var sg; var sb; var sa
    var br; var bg; var bb; var ba
    ...
    this.process = function(mode, srcContext, dstContext, alpha, offset) {
      ...
      if (normal || nativeModes[mode]) {
        ...
        var src = srcContext.getImageData(0, 0,
  ...
  new function() {
    ...
    function getTransform(matrix, coordinates, center) {
      ...
      if (!matrix.isIdentity()) {
        ...
        if (decomposed) {
          ...
          var scale = decomposed.scaling
          var skew = decomposed.skewing
          if (trans && !trans.isZero()) { parts.push('translate(' + formatter.point(trans) + ')') }
          if (angle) { parts.push('rotate(' + formatter.number(angle) + ')') }
          if (!Numerical.isZero(scale.x - 1) ||
						!Numerical.isZero(scale.y - 1)) { parts.push('scale(' + formatter.point(scale) + ')') }
    ...
    function exportShape(item) {
      ...
      var attrs = getTransform(item._matrix, true, type !== 'rectangle')
      if (type === 'rectangle') {
        type = 'rect'
        var size = item._size
        var width = size.width
    ...
    function exportSymbolItem(item, options) {
      var attrs = getTransform(item._matrix, true)
      var definition = item._definition
      var node = getDefinition(definition, 'symbol')
      var definitionItem = definition._item
      var bounds = definitionItem.getStrokeBounds()
      if (!node) {
    ...
    function exportGradient(color) {
      ...
      if (!gradientNode) {
        ...
        var radial = gradient._radial
        var origin = color.getOrigin()
        var destination = color.getDestination()
        var attrs
        if (radial) {
          attrs = {
            cx: origin.x,
        ...
        for (var i = 0, l = stops.length; i < l; i++) {
          var stop = stops[i]
          var stopColor = stop._color
          var alpha = stopColor.getAlpha()
          var offset = stop._offset
          attrs = {
    ...
    function applyStyle(item, node, isRoot) {
      var attrs = {}
      var parent = !isRoot && item.getParent()
      var style = []
    ...
    function exportSVG(item, options, isRoot) {
      ...
      var node = exporter && exporter(item, options)
      if (node) {
    ...
    Project.inject({
      exportSVG: function(options) {
        ...
        var bounds = Base.pick(options.bounds, 'view')
        var mx = options.matrix || bounds === 'view' && view._matrix
        var matrix = mx && Matrix.read([mx])
        var rect = bounds === 'view'
  ...
  new function() {
    ...
    function importGroup(node, type, options, isRoot) {
      ...
      var isClip = type === 'clippath'
      var isDefs = type === 'defs'
      var item = new Group()
      var project = item._project
      var currentStyle = project._currentStyle
      var children = []
      if (!isClip && !isDefs) {
      ...
      for (var i = 0, l = nodes.length; i < l; i++) {
        ...
        if (childNode.nodeType === 1 &&
					...
					!(child instanceof SymbolDefinition)) { children.push(child) }
      }
    ...
    function importGradient(node, type) {
      ...
      var radial = type === 'radialgradient'
      var gradient
      if (id) {
      ...
      var origin; var destination; var highlight
      var scaleToBounds = getValue(node, 'gradientUnits', true) !==
				'userSpaceOnUse'
      if (radial) {
    ...
    var attributes = Base.set(Base.each(SvgStyles, function(entry) {
      ...
      viewBox: function(item, value, name, node, styles) {
        ...
        var size = getSize(node, null, null, true)
        var group
        var matrix
        if (item instanceof Group) {
          var scale = size ? size.divide(rect.getSize()) : 1
          var matrix = new Matrix().scale(scale)
    ...
    function getDefinition(value) {
      ...
      var res = name && definitions[window
    ...
    function importNode(node, options, isRoot) {
      ...
      var settings = paper.settings
      var applyMatrix = settings.applyMatrix
      var insertItems = settings.insertItems
      settings.applyMatrix = false
      settings.insertItems = false
      var importer = importers[type]
      var item = importer && importer(node, type, options, isRoot) || null
      settings.insertItems = insertItems
      settings.applyMatrix = applyMatrix
      if (item) {
        if (isElement && !(item instanceof Group)) { item = applyAttributes(item, node, isRoot) }
        var onImport = options.onImport
        var data = isElement && node.getAttribute('data-paper-data')
  ...
  Base.exports.PaperScript = function() {
    ...
    if (!acorn) {
	  ...
	  skipSpace()
        ...
        var _num = { type: 'num' }; var _regexp = { type: 'regexp' }; var _string = { type: 'string' }
        var _name = { type: 'name' }; var _eof = { type: 'eof' }

        var _break = { keyword: 'break' }; var _case = { keyword: 'case', beforeExpr: true }; var _catch = { keyword: 'catch' }
        var _continue = { keyword: 'continue' }; var _debugger = { keyword: 'debugger' }; var _default = { keyword: 'default' }
        var _do = { keyword: 'do', isLoop: true }; var _else = { keyword: 'else', beforeExpr: true }
        var _finally = { keyword: 'finally' }; var _for = { keyword: 'for', isLoop: true }; var _function = { keyword: 'function' }
        var _if = { keyword: 'if' }; var _return = { keyword: 'return', beforeExpr: true }; var _switch = { keyword: 'switch' }
        var _throw = { keyword: 'throw', beforeExpr: true }; var _try = { keyword: 'try' }; var _var = { keyword: 'var' }
        var _while = { keyword: 'while', isLoop: true }; var _with = { keyword: 'with' }; var _new = { keyword: 'new', beforeExpr: true }
        var _this = { keyword: 'this' }

        var _null = { keyword: 'null', atomValue: null }; var _true = { keyword: 'true', atomValue: true }
        ...
        var _bracketL = { type: '[', beforeExpr: true }; var _bracketR = { type: ']' }; var _braceL = { type: '{', beforeExpr: true }
        var _braceR = { type: '}' }; var _parenL = { type: '(', beforeExpr: true }; var _parenR = { type: ')' }
        var _comma = { type: ',', beforeExpr: true }; var _semi = { type: ';', beforeExpr: true }
        var _colon = { type: ':', beforeExpr: true }; var _dot = { type: '.' }; var _question = { type: '?', beforeExpr: true }

        var _slash = { binop: 10, beforeExpr: true }; var _eq = { isAssign: true, beforeExpr: true }
        var _assign = { isAssign: true, beforeExpr: true }
        var _incDec = { postfix: true, prefix: true, isUpdate: true }; var _prefix = { prefix: true, beforeExpr: true }
	  ...
	  compareTo(words)
        ...
        function skipBlockComment() {
          ...
          var start = tokPos; var end = input.indexOf('*/', tokPos += 2)
	  ...
	  raise(tokPos, "Unexpected character '" + ch + "'")
        ...
        function readRegexp() {
          var content = ''; var escaped; var inClass; var start = tokPos
          for (;;) {
	  ...
	  raise(e)
        ...
        function readInt(radix, len) {
          var start = tokPos; var total = 0
          for (var i = 0, e = len == null ? Infinity : len; i < e; ++i) {
	  var code = input.charCodeAt(tokPos); var val
	  ...
	  total = total * radix + val
        ...
        function readNumber(startsWithDot) {
          var start = tokPos; var isFloat = false; var octal = input.charCodeAt(tokPos) === 48
	  ...
	  isFloat = true
          ...
          var str = input.slice(start, tokPos); var val
	  ...
	  if (ch === 92) {
        ...
        function readWord1() {
          containsEsc = false
          var word; var first = true; var start = tokPos
          for (;;) {
	  var ch = input.charCodeAt(tokPos)
	  ...
	  (tokType === _eof || tokType === _braceR || newline.test(input.slice(lastEnd, tokStart)))
        ...
        function parseTopLevel(program) {
          ...
          var node = program || startNode(); var first = true
	  ...
	  first = false
        ...
        function parseStatement() {
          ...
          var starttype = tokType; var node = startNode()
	  ...
	  return finishNode(node, 'EmptyStatement')
            ...
            default:
	  var maybeName = tokVal; var expr = parseExpression()
	  if (starttype === _name && expr.type === 'Identifier' && eat(_colon)) {
        ...
        function parseBlock(allowStrict) {
          var node = startNode(); var first = true; var strict = false; var oldStrict
          node.body = []
          expect(_braceL)
          while (!eat(_braceR)) {
	  var stmt = parseStatement()
	  ...
	  if (prec > minPrec) {
        ...
        function parseMaybeUnary() {
          if (tokType.prefix) {
	  var node = startNode(); var update = tokType.isUpdate
	  node.operator = tokVal
	  node.prefix = true
	  tokRegexpAllowed = true
	  next()
	  ...
	  if (update) checkLVal(node.argument)
			   ...
			   node.argument.type === 'Identifier') { raise(node.start, 'Deleting local variable in strict mode') }
	  ...
	  return finishNode(node, 'Literal')
            ...
            case _parenL:
	  var tokStartLoc1 = tokStartLoc; var tokStart1 = tokStart
	  next()
	  var val = parseExpression()
	  val.start = tokStart1
	  val.end = tokEnd
	  ...
	  unexpected()
        ...
        function parseObj() {
          var node = startNode(); var first = true; var sawGetSet = false
          node.properties = []
          next()
          while (!eat(_braceR)) {
	  if (!first) {
	  ...
	  var prop = { key: parsePropertyName() }; var isGetSet = false; var kind
	  if (eat(_colon)) {
	  ...
	  node.params.push(parseIdent())
          ...
          var oldInFunc = inFunction; var oldLabels = labels
	  ...
	  if (!liberal &&
		  ...
		  input.slice(tokStart, tokEnd).indexOf('\\') == -1) { raise(tokStart, "The keyword '" + tokVal + "' is reserved") }
	  node.name = tokVal
    ...
    function compile(code, options) {
      ...
      function replaceCode(node, str) {
        ...
        var end = getOffset(node.range[1])
        var insert = 0
        ...
        switch (node.type) {
          ...
          case 'BinaryExpression':
            if (node.operator in binaryOperators &&
              ...
              var between = getBetween(node.left, node.right)
          ...
          case 'AssignmentExpression':
            ...
            if (!(
              ...
              if (node.type === 'UpdateExpression') {
                var arg = getCode(node.argument)
                var exp = '__$__(' + arg + ', "' + node.operator[0] +
									'", 1)'
                var str = arg + ' = ' + exp
                if (node.prefix) {
                  str = '(' + str + ')'
      ...
      var agent = paper.agent
      var version = agent.versionNumber
      var offsetCode = false
      var lineBreaks = /\r\n|\n|\r/mg
      var map
    ...
    function execute(code, scope, options) {
      ...
      var tool = /\btool\.\w+|\s+on(?:Key|Mouse)(?:Up|Down|Move|Drag)\b/
        .test(code) && !/\bnew\s+Tool\b/.test(code)
        ? new Tool() : null
      var toolHandlers = tool ? tool._events : []
      var handlers = ['onFrame', 'onResize'].concat(toolHandlers)
      var params = []
      var args = []
      var func
      var compiled = typeof code === 'object' ? code : compile(code, options)
      code = compiled.code
    ...
    function loadScript(script) {
      if (/^text\/(?:x-|)paperscript$/.test(script.type) &&
        ...
        var async = PaperScope.hasAttribute(script, 'async')
        var scopeAttribute = 'data-paper-scope'
        if (!canvas) {
