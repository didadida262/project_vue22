...
export default {
  ...
  methods: {
    ...
    drawInnerCircle(info, radius) {
      const xBot = info.DOWN * this.ratio / 2
      ...
      if (info.LEFT) {
        // 若还存在left，需额外操作
        const yLeft = info.LEFT / 2
        const xLeft = getAnotherPoint(yLeft, radius)
        const leftTop = new paper.Point(-xLeft, -yLeft)
        const leftBot = new paper.Point(-xLeft, yLeft)
        const throughleft = new paper.Point(0, -radius)
        const ppp = new paper.Path.Arc({
          from: leftTop,
          through: throughleft,
          to: leftBot,
          strokeColor: 'rgb(150,150,150)',
    ...
    drawData() {
      ...
      const currentProject = this.paper.projects.filter((item) => item.name === 'circle')[0]
      currentProject.activate()
      const layerDot = currentProject.layers['layerData']
      if (!layerDot) {
      ...
      dotData.forEach((item) => {
        ...
        p.onClick = (e) => {
          if (this.$route.name !== 'portalmain') { this.setSelectedCenter({ id: item.id, point: preDealPoint(e.point, this.ratio), from: 'circle' }) }
        }
      })
    ...
    topLayerData() {
      const currentProject = this.paper.projects.filter((item) => item.name === 'circle')[0]
      const layerData = currentProject.layers['layerData']
      currentProject.insertLayer(currentProject.layers.length - 1, layerData)
    },
    // 框选区域次顶
    secondLayerPoint() {
      const currentProject = this.paper.projects.filter((item) => item.name === 'circle')[0]
      const layerPointer = currentProject.layers['layerPointer']
    ...
    drawRealtimeScanningBox() {
      ...
      const currentProject = this.paper.projects.filter((item) => item.name === 'circle')[0]
      currentProject.activate()
      const layerScanningBox = currentProject.layers['layerScanningBox']
      if (!layerScanningBox) {
        const layerScanningBox = new paper.Layer()
  ...
  beforeDestroy() {
    const project = this.paper.projects.filter((item) => item.name === 'circle')[0]
    project.remove()
  },
  computed: {
    ...
    selectedArea(val, old) {
      ...
      const operation = {
        ...
        ratio: () => {
          ...
          const oldLayer = this.currentProject.layers.filter((_layer) => _layer.name === 'layerPointer')[0]
          oldLayer.remove()
